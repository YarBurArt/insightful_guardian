...
async def create_post(request: Request, post: Post):
    ...
    none_check_with_msg(res, "Post not created, clean error :)")
...
async def get_all_posts():
    ...
    none_check_with_msg(res, "Posts not found in DB, try add posts")
...
async def get_posts_by_category(category: str):
    ...
    none_check_with_msg(res, "Posts not found in DB, try another category")
...
async def search_posts(query: str):
    ...
    none_check_with_msg(res, "Posts not found in DB, try another query")
    return JSONResponse(example_posts, status_code=200)
